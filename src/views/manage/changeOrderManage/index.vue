<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="manageOrdersList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="改签要求" align="center" prop="note" show-overflow-tooltip />
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          ...
          <el-button link type="primary" icon="View" @click="handleView(scope.row)" v-hasPermi="['manage:manageOrders:query']">查看订单详细</el-button>
           <el-button link type="primary" icon="Switch"  @click="handleUpdate(scope.row)" v-hasPermi="['manage:manageOrders:query']">处理改签</el-button>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-card class="change-order-card">
        <div class="card-header">
          <h3>订单改签</h3>
          <p class="order-id">订单号: {{ form.orderId }}</p>
        </div>
        
        <el-divider></el-divider>
        
        <el-form ref="manageOrdersRef" :model="form" :rules="rules" label-width="100px" label-position="top">
          <el-form-item label="原出发日期">
            <el-input v-model="form.departureDate" disabled>
              <template #prepend>
          ...
          <el-form-item label="可选出发日期" prop="newDepartureDate">
            <el-select 
              v-model="form.newDepartureDate" 
              placeholder="请选择新的出发日期" 
              clearable
              style="width: 100%"
            >
              <el-option
                v-for="date in departureDateList"
                :key="date"
          ...
          <el-form-item label="改签备注" prop="note">
            <el-input 
              v-model="form.note" 
              type="textarea" 
              placeholder="请输入改签备注（选填）" 
              :rows="3"
            />
          </el-form-item>
        </el-form>
    ...
    <el-dialog :title="detailTitle" v-model="detailOpen" width="600px" append-to-body>
      <el-card :class="['order-detail-card', getOrderStatusClass(detailForm.orderStatus)]">
        ...
        <div class="note-section">
          <div class="info-item">
            <span class="info-label">改签备注:</span>
...
import { listManageOrders, getManageOrders, delManageOrders, addManageOrders, updateManageOrders } from "@/api/manage/manageOrders";
...
function getList() {
  loading.value = true;
  queryParams.value.orderStatus = '6'
...
function handleUpdate(row) {
  ...
  getManageOrders(_orderId).then(response => {
    ...
    // 添加新字段用于存储新选择的出发日期
    form.value.newDepartureDate = null;
    getProductInventoryDate(form.value.productId).then(res => {
      // 将获取到的出发日期数据存储到变量中
...
function submitForm() {
  proxy.$refs["manageOrdersRef"].validate(valid => {
    if (valid) {
      if (form.value.newDepartureDate) {
        // 更新出发日期为新选择的日期
        form.value.departureDate = form.value.newDepartureDate;
        // 移除临时字段
        delete form.value.newDepartureDate;
        
        if (form.value.orderId != null) {
          form.value.orderStatus = '2'; // 将订单状态设置为已确认
          updateManageOrders(form.value).then(response => {
            proxy.$modal.msgSuccess("改签成功");
            open.value = false;
            getList();
          });
        } else {
        ...
        proxy.$modal.msgWarning("请选择新的出发日期");
      }
    }
  });
}
